...
<div class="quickfilters">
  ...
  <span class="sep">|</span>
  <a href="/" style="color:#7af;">&#9654; Torrent Search</a>
</div>
    ...
    <tr>
      ...
      <td>
        ...
        <div class="scene-site">
          ...
          {% endif %}
          {% if s.date %}<span class="muted">{{ s.date.isoformat() }}</span>{% endif %}
...
def scenes_ui(
    ...
    for s in scenes:
        ...
        for t in s.get("torrents") or []:
            ...
            raw_date = t.get("date_added")
            if isinstance(raw_date, str):
                from datetime import datetime, timezone
                try:
                    raw_date = datetime.fromisoformat(raw_date).replace(tzinfo=timezone.utc)
                except ValueError:
                    raw_date = None
            t["age_label"] = format_date(raw_date)
    ...
    html = tmpl.render(
        ...
        has_prev=page > 1,
        has_next=page < total_pages,
        prev_url="/scenes?" + urlencode({k: v for k, v in {**base_args, "page": page - 1}.items() if v}),
        next_url="/scenes?" + urlencode({k: v for k, v in {**base_args, "page": page + 1}.items() if v}),
    )
    return HTMLResponse(content=html)
